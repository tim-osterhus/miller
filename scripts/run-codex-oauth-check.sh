...
script_path="$repo_root/scripts/run-codex-oauth-check.sh"
bundle_root="$repo_root/.artifacts/Miller.app"
run_root="$repo_root/.artifacts/codex-oauth-check"
...
validate() {
  zsh -n "$script_path"
  grep -Fqx 'Result: PASS (2026-07-31)' \
    "$repo_root/docs/qualification/provider-check.md"
  grep -Fq 'A browser-based Codex OAuth login will open' "$script_path"
  grep -Fq 'MILLER_CODEX_OAUTH_CHECK_CONFIRM' "$script_path"
  ...
  printf 'MILLER_CODEX_OAUTH_CHECK_VALID\n'
}
...
dry_run() {
  ...
  printf 'MILLER_CODEX_OAUTH_CHECK_DRY_RUN\n'
}
...
if [[ "$mode" != "--interactive" || "$confirmation" != "--confirm-live" || $# != 2 ]]; then
  printf '%s\n' \
    'REFUSED: connected Codex qualification is opt-in.' \
    "usage: $0 --interactive --confirm-live" >&2
  exit 64
fi

printf '%s\n' \
  'A browser-based Codex OAuth login will open.' \
  'This may read and replace the Miller Codex credential in Keychain.' \
  'Only PASS/FAIL status and elapsed timing facts are retained temporarily.' \
  'Callback URLs, account identifiers, tokens, prompts, and provider text are forbidden.'
printf 'Type MILLER_CODEX_OAUTH_CHECK_CONFIRM to continue: '
read -r answer
[[ "$answer" == "MILLER_CODEX_OAUTH_CHECK_CONFIRM" ]] || {
  printf 'MILLER_CODEX_OAUTH_CHECK_CANCELLED\n'
  exit 64
}
...
printf '%s\n' \
  'Use Miller Settings to complete the Codex OAuth checklist in' \
...
printf 'result=%s\n' "$result" > "$status_path"
printf 'MILLER_CODEX_OAUTH_CHECK_%s\n' "$result"
